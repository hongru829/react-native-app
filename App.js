...
import Expo, { Constants, Asset, Audio, Font, Video } from 'expo';
...
export default class App extends React.Component {
  ...
  onNavigationStateChange = (navState) => {
    ...
    if (navState.url.slice(0,22) === 'about://callback/login') {
      ...
      const regex = /^about:\/\/callback\/login\?code=(.+)&state=random_string/
      let authorization_code = navState.url.match(regex)[1]
      const url = 'https://api.npr.org/authorization/v2/token'
      fetch(url,
        {
          body: `grant_type=authorization_code&client_id=nprone_trial_Oj57ObyP7fOd&client_secret=0evhK80EWBKy9dzsh2qg06Tekr4zqjVo2xBAoJXt&code=${authorization_code}&redirect_uri=about%3A%2F%2Fcallback%2Flogin`,
          headers: {
        ...
        .then((responseJson) => {
            ...
            if (access_token) {
              ...
              .then(response => response.json())
              .then(responseJson => {
                const mediaItems = responseJson["items"]
                for (let i = 0; i < mediaItems.length; i++) {
                  let title = mediaItems[i]["attributes"]["title"]
                  let uri = mediaItems[i]["links"]["audio"][0]["href"]
                  let playListItem = new PlaylistItem(title, uri, false)
  ...
  render() {
    ...
    const client_id = 'nprone_trial_Oj57ObyP7fOd'
    const redirect_uri = 'about%3A%2F%2Fcallback%2Flogin'
    const scope = 'listening.readonly'
    const state = 'random_string'
    const loginUrl = `https://api.npr.org/authorization/v2/authorize?client_id=${client_id}&redirect_uri=${redirect_uri}&response_type=code&scope=${scope}&state=${state}`
    return !this.state.fontLoaded || this.state.access_token === null
      ? <WebView source={{uri: loginUrl}} onNavigationStateChange={this.onNavigationStateChange} />
